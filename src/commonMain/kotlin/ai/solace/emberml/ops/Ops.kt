...
object Ops {
     ...
     */
    fun getBackend(): String = ai.solace.emberml.backend.getBackend()
    
    /**
     * Set the active backend.
     */
    fun setBackend(name: String): Boolean = ai.solace.emberml.backend.setBackend(name)
    
    /**
     * Automatically select and set the best backend based on hardware.
     */
    fun autoSelectBackend(): String = ai.solace.emberml.backend.autoSelectBackend()
     ...
     */
    fun cast(tensor: EmberTensor, dtype: EmberDType): EmberTensor {
        val backend = BackendRegistry.getCurrentBackend()
